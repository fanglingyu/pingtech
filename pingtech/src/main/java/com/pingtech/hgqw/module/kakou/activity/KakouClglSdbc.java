...
public class KakouClglSdbc extends MyActivity implements OnHttpResult, OffLineResult {
	...
	protected void onCreate(Bundle savedInstanceState) {
		String title = getString(R.string.kakoumanager) + ">" + getString(R.string.readcard_cljc_kakou) + ">" + getString(R.string.sendPassInfo);
...
//		cllx_str = cllx.getText().toString();
	...
	private void sendPassInfo() {
		...
		if (from == ManagerFlag.PDA_KKGL_CLYZ) {
			...
			params.add(new BasicNameValuePair("txkkmc", (String) bindData.get("kkmc")));
		...
		params.add(new BasicNameValuePair("gs_yyz", gs_yyz_str)); 
		params.add(new BasicNameValuePair("gjdq", gjdq_str)); 
		...
		params.add(new BasicNameValuePair("lxfs", lxfs_str)); 
		...
		if (!Utils.getState(FunctionSetting.bdtxyz, true)) {
			...
			url = "sendClPassInfo";
			OffLineManager.request(this, new KakouAction(), url, NVPairTOMap.nameValuePairTOMap(params), 0);
		}
	}
