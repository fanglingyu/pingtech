...
public class MyActivity extends Activity {
	...
	/**
	 * 
	 * 每个界面再创建时，先都按照base.xml创建一个layout，
	 * 然后再把每个activity定义的layout加到该layout的baselayout下
	 * 
	 * @param layoutResID
	 ...
	 * 
	 * @方法名：checkMtBwQy
	 * @功能说明：校验内存中的码头、泊位、区域信息
	 * @author liums
	 * @date 2013-12-17 下午4:03:38
	 */
	private void checkMtBwQy() {
		if (BaseInfoData.mDockList == null || BaseInfoData.mDockList.size() < 1 || BaseInfoData.mBerthList == null
			...
			Log.i("MyActivity", "MtBwQy has null ,checkMtBwQy");
			File file = new File(BaseApplication.instent.getLocalPath() + "baseinfo.xml");
			if (!file.exists() && BaseApplication.instent.getWebState()) {// 如果不存在本地文件，并且网络可用，则请求服务器
	...
	// /*
	// * switch (keyCode) { case KeyEvent.KEYCODE_HOME: activityForHome();//
	// * 拦截HOME键 return false; default: break; }
	// */
	// return super.onKeyDown(keyCode, event);
	// }
	//
	// public boolean onKeyDown(int keyCode, KeyEvent event, boolean isControl)
	// {
	// if (isControl) {
	 ...
	 * @功能说明：开启Mqtt服务
	 * @author zhaotf
	 * @date 2014-3-27 下午12:02:08
	 */
	protected void openMqttService() {
	...
	/**
	 * 
	 * @方法名：closeMqttService
	 * @功能说明：关闭Mqtt服务
	 * @author zhaotf
	 * @date 2014-3-27 下午12:02:25
	 */
	protected void closeMqttService() {
	 ...
	 * 
	 * @方法名：getState
	 * @功能说明：得到SharedPreferences里面存的值
	 * @author zhaotf
	 * @date 2014-3-27 下午12:10:53
	 * @param key
	 *            键
	 * @param def
	 *            默认值
	 * @return
	 */
	protected boolean getState(String key, boolean def) {
	...
	private void activityForHome() {
		...
		intent.putExtra("activityForHome", true);
