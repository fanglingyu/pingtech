...
public class M1CardAPI {
	...
	public byte[] buffer = new byte[120];
	 ...
	 * @return
	 */
	public Result validatePassword(int position, int keyType, byte[] password) {
		...
		// 验证密码
		byte[] command2 = (DATA_PREFIX + PASSWORD_VALIDATE_ORDER + getKeyTypeStr(keyType) + getZoneId(position) + ENTER).getBytes();
	...
	public byte[][] read(int startPosition, int num) {
		byte[] command = { 'c', '0', '5', '0', '6', '0', '5', '0', '4', '0', '0', '\r', '\n' };
		...
		for (int i = 0; i < num; i++) {
			...
			if (split.length == 2) {
				...
				// Log.i("whw","split msg=" + msg + "  msg length=" +
				// msg.length());
			}
