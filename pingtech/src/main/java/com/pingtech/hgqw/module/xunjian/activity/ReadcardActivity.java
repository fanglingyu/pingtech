 ...
 * */
public class ReadcardActivity extends MyActivity implements OnHttpResult,
		OffLineResult {
	...
	private static final int HTTPREQUEST_TYPE_FOR_NORMAL_XUNJIAN_ID_OFFLINE = 10000;
	...
	/* 循环读二代证，loopread为false时退出 */
	private boolean loopread = false;
		...
		public void handleMessage(Message msg) {
			...
			if (StringUtils.isEmpty(zxingInfo)) {
				HgqwToast.getToastView(getApplicationContext(),
						getString(R.string.cardnum_empty)).show();
				doingZxing = false;
				// 初始化二维码扫描结果对象
				msTdc = null;
				return;
	...
	private OnClickListener retryInputCardNumListener = new OnClickListener() {
		public void onClick(View v) {
			...
			case DeviceUtils.DEVICE_MODEL_MIMA:
				((Button) findViewById(R.id.xcxj_rcxj_select_place))
			...
			if (bindType != -1) {
				 ...
				 * 二维码扫描按钮
				 */
				if (findViewById(R.id.readcard_btnzxing) != null) {
					findViewById(R.id.readcard_btnzxing).setVisibility(
							View.VISIBLE);
				}
				if (findViewById(R.id.readcard_zxing) != null) {
					findViewById(R.id.readcard_zxing).setVisibility(
							View.VISIBLE);
				}
	...
	private OnClickListener exceptRegistListener = new OnClickListener() {
		public void onClick(View v) {
			...
			if (res != R.id.btnExceptionRegist_object) {
				...
				if (SystemSetting.xunJianId != null
	...
	/**
	 * 巡查巡检下的日常巡检时，界面上有三个选项：日常巡检、查岗查哨、巡查地点，此方法用于监听点击哪个checkbox。
	 * radio_btn_ic表示巡查地点； radio_btn_id表示日常巡检； radio_btn_id_guard表示查岗查哨
	 * */
	private OnCheckedChangeListener listener = new OnCheckedChangeListener() {
		@Override
		public void onCheckedChanged(RadioGroup group, int checkedId) {
			if (checkedId == R.id.radio_btn_ic) {
				((TextView) findViewById(R.id.berthcard))
						.setText(R.string.iccardnum);

				if (findViewById(R.id.payTypeTitle) != null) {
					((TextView) findViewById(R.id.payTypeTitle))
							.setText(R.string.readInfoLabelData);
				}
				// 巡检地点按钮，关闭身份证读卡器，启动IC读卡器
				readMode = ICReadEntity.READ_TYPE_DEFAULT;
				//
				((Button) findViewById(R.id.btn_ic_id_change))
						.setVisibility(View.GONE);
				((Button) findViewById(R.id.xcxj_rcxj_select_place))
						.setVisibility(View.VISIBLE);
				...
				sbz = false;
			} else if (checkedId == R.id.radio_btn_id
					|| checkedId == R.id.radio_btn_id_guard) {
				switch (DeviceUtils.getDeviceModel()) {
				case DeviceUtils.DEVICE_MODEL_MIMA:
					((Button) findViewById(R.id.btn_ic_id_change))
							.setVisibility(View.VISIBLE);
				...
				if (checkedId == R.id.radio_btn_id) {
					sbz = false;
				} else {
					sbz = true;
				}
				((TextView) findViewById(R.id.berthcard))
						.setText(R.string.idcardnum);
				if (findViewById(R.id.payTypeTitle) != null) {
					((TextView) findViewById(R.id.payTypeTitle))
							.setText(R.string.readCardData);
				}
				/*
				 * if
				 * (!Environment.getExternalStorageState().equals(Environment.
				 * MEDIA_MOUNTED)) { Toast.makeText(ReadcardActivity.this,
				 ...
				 * R.string.reader_initcomm_failure,
				 * BaseToast.LENGTH_LONG).show(); return; }
				 */
				if (input != null) {
					input.setText("");
				}
				readMode = ICReadEntity.READ_TYPE_SAFE;// 读加密区
	...
	public void zxingScanMethod(View v) {
		...
		default:
			// 二维码扫描结果对象初始化
			msTdc = null;
			Intent intent = new Intent(ReadcardActivity.this,
	...
	private EditText.OnKeyListener keyListener = new EditText.OnKeyListener() {
		...
		public boolean onKey(View v, int keyCode, KeyEvent event) {
			Log.i(TAG, "EditText.OnKeyListener onKey");
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		cbzwm = intent.getStringExtra("cbzwm");
		...
		if (from != null && from.equals("04")) {
			super.onCreate(savedInstanceState, R.layout.persondetail);
			...
			yxq_s = intent.getStringExtra("yxq");
			if (photo_s != null && !"".equals(photo_s)) {
				byte[] image = Base64.decode(photo_s);
				BitmapFactory.Options opts = new BitmapFactory.Options();
				opts.inJustDecodeBounds = true;
				netWorkImage = BitmapFactory.decodeByteArray(image, 0,
						image.length, opts);
			...
			onUpdateDisplayInfo(true, true);
		} else {
			super.onCreate(savedInstanceState, R.layout.readcard);
			input = (EditText) findViewById(R.id.cardtext);
			RadioGroup rg = (RadioGroup) findViewById(R.id.readcard_radio);
			rg.setOnCheckedChangeListener(listener);
			if (cardType == READCARD_TYPE_ID_CARD) {
			...
			if (bindType != -1) {
				...
				// 船舶绑定---启动IC读卡器
				if (/* bindType == GlobalFlags.LIST_TYPE_FROM_SHIPSTATUS && */cardType == ReadcardActivity.READCARD_TYPE_IC_CARD) {
					// 船舶绑定读默认区域，刷卡读加密区域
					readMode = ICReadEntity.READ_TYPE_DEFAULT;
		...
		zxingButton = (Button)findViewById(R.id.readcard_btnzxing);
		zxingTextView = (TextView)findViewById(R.id.readcard_zxing);
	}

	private void sbGoodsCheck() {
		findViewById(R.id.tikou_downup_ship_imageview).setVisibility(
				View.VISIBLE);
		...
		HashMap<String, Object> tikoumBindShip = SystemSetting
				.getBindShip(GlobalFlags.LIST_TYPE_FROM_TIKOUMANAGER + "");
		...
		ArrayAdapter<String> shipAdapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_spinner_item, cbzwmList);
		shipAdapter
	...
	private void onReadComplete(boolean idcard, String inputType) {
		...
		if ("02".equals(from) || "01".equals(from)) {
			...
			if (StringUtils.isNotEmpty(inputType)) {
				if ("0".equals(inputType.trim())
						|| "1".equals(inputType.trim())
						|| "2".equals(inputType.trim())) {
			...
			params.add(new BasicNameValuePair("acrossTime",
			...
			NetWorkManager.request(this, url, params,
					HTTPREQUEST_TYPE_FOR_TRAFFIC_VALID);
		} else if ("03".equals(from) || "04".equals(from)) {
			mIdcard = idcard;
			RadioGroup rg = (RadioGroup) findViewById(R.id.readcard_radio);
			if ((rg != null)
					&& (rg.getCheckedRadioButtonId() == R.id.radio_btn_ic)) {
				// 电子标签
				...
				progressDialog.show();
				NetWorkManager.request(this, url, params,
						HTTPREQUEST_TYPE_FOR_NORMAL_XUNJIAN_IC);
				loopread = false;
			} else {
				// 刷卡
				HashMap<String, Object> Binddata = SystemSetting
						.getBindShip(GlobalFlags.LIST_TYPE_FROM_XUNCHAXUNJIAN
								+ "");
				/*
				 * if (Binddata == null && (SystemSetting.xunJianId == null ||
				 * SystemSetting.xunJianId.length() == 0)) {
				 * MyToast.getToastView(getApplicationContext(),
				 * getString(R.string.found_not_addr)).show(); return; }
				 */
				...
				if (Binddata != null) {
					...
					params.add(new BasicNameValuePair("berthcode",
							(String) Binddata.get("tkbw")));
					params.add(new BasicNameValuePair("dockcode",
							(String) Binddata.get("tkmt")));
				} else {
					params.add(new BasicNameValuePair("voyageNumber",
							voyageNumber));
					params.add(new BasicNameValuePair("kacbqkid", kacbqkid));
					params.add(new BasicNameValuePair("cbzwm", cbzwm));
				}
				...
				if (StringUtils.isNotEmpty(inputType)) {
					if ("0".equals(inputType.trim())
							|| "1".equals(inputType.trim())
							|| "2".equals(inputType.trim())) {
						params.add(new BasicNameValuePair("sfsk", inputType
								.trim()));
					} else {
						params.add(new BasicNameValuePair("sfsk", "0"));
					}
				...
				if (!getState(FunctionSetting.bdtxyz, true)) {
					if (progressDialog != null) {
						progressDialog
								.setMessage(getString(R.string.offline_off_request_web));
					}
					NetWorkManager.request(this, url, params,
							HTTPREQUEST_TYPE_FOR_NORMAL_XUNJIAN_ID);
				} else {
					OffLineManager.request(this, new XunJianAction(), url,
							NVPairTOMap.nameValuePairTOMap(params),
							HTTPREQUEST_TYPE_FOR_NORMAL_XUNJIAN_ID_OFFLINE);
				}
				loopread = false;
			}
		} else if (GlobalFlags.BINDSHIP_FROM_KAKOUMANAGER.equals(from)
				|| GlobalFlags.BINDSHIP_FROM_TIKOUMANAGER.equals(from)
				|| GlobalFlags.BINDSHIP_FROM_XUNCHAXUNJIAN.equals(from)
				|| GlobalFlags.BINDSHIP_FROM_SHIPSTATUS.equals(from)) {// 船舶动态---船舶绑定
					...
					if ("result".equals(parser.getName())) {
						...
						if (zjxx) {
							...
							if (icpic_s != null && icpic_s.length() > 0) {
								...
								BitmapFactory.Options opts = new BitmapFactory.Options();
								opts.inJustDecodeBounds = true;
								netWorkImage = BitmapFactory.decodeByteArray(
										image, 0, image.length, opts);
								int height_be = opts.outHeight / 130;
								int width_be = opts.outWidth / 105;
	...
	public void onHttpResult(String str, int httpRequestType) {
		...
		if (httpRequestType == HTTPREQUEST_TYPE_FOR_TRAFFIC_VALID) {
			...
			hasImage = false;
			successFlag = 0;
			time_s = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
					.format(new Date(System.currentTimeMillis()));
			success = onParseXMLData(str);
			if (!success) {
				HgqwToast.getToastView(getApplicationContext(),
						tempHttpReturnXMLInfo).show();
				if (old_idcardnumber_s.length() > 0) {
					idcardnumber_s = old_idcardnumber_s;
					old_idcardnumber_s = "";
				}
			...
			if (hasCardInfo == false) {
				...
				startActivityForResult(intent,
						STARTACTIVITY_FOR_REGISTER_PERSONINFO);
				doingZxing = false;
				return;
			}
			setContentView(R.layout.persondetail);

			onUpdateDisplayInfo(success, true);
			// 修改通行方向按钮和声音控制。
			if (successFlag == 1) {
			...
			if (success) {
				if (sxcfx_s.equals("0")) {
					if (from.equals("02")) {
						((TextView) findViewById(R.id.Traffic_direction))
								.setText(Html
										.fromHtml(getString(R.string.Traffic_direction)
												+ "<font color=\"#acacac\">"
												+ "下船" + "</font>"));
					} else {
						((TextView) findViewById(R.id.Traffic_direction))
								.setText(Html
										.fromHtml(getString(R.string.Traffic_direction_kk)
												+ "<font color=\"#acacac\">"
												+ "出卡口" + "</font>"));
					}
					findViewById(R.id.imageView_Traffic_direction)
							.setVisibility(View.VISIBLE);
					((ImageView) findViewById(R.id.imageView_Traffic_direction))
				...
				HgqwToast.getToastView(getApplicationContext(),
						getString(R.string.modify_success)).show();
				((Button) findViewById(R.id.btnmodifyPassDirection))
						.setEnabled(false);
			} else {
				HgqwToast.getToastView(getApplicationContext(),
						getString(R.string.modify_failure)).show();
			}

		} else if (HTTPREQUEST_TYPE_FOR_NORMAL_XUNJIAN_ID == httpRequestType) {
			if (progressDialog != null) {
				progressDialog.dismiss();
				progressDialog = null;
			}
			...
			if (StringUtils.isEmpty(str)
					&& !getState(FunctionSetting.bdtxyz, true)) {
				HgqwToast.getToastView(getApplicationContext(),
						getString(R.string.data_download_failure_info)).show();
				if (!from.equals("04")) {
					if (old_idcardnumber_s.length() > 0) {
			...
			// 解析XML
			success = onParseXunJianXMLData(str);
			if (!success) {
				HgqwToast.getToastView(getApplicationContext(),
						tempHttpReturnXMLInfo).show();
				if (!from.equals("04")) {
			...
			if (!sbz) {
				...
				// 巡检模块证件号码字段单独处理
				idcardnumber_s = zjhm;
				onUpdateDisplayXunjianInfo(success, true, false);
			...
			// 初始化mstdc(二维码扫描结果对象)
			msTdc = null;
		} else if (HTTPREQUEST_TYPE_FOR_NORMAL_XUNJIAN_ID_OFFLINE == httpRequestType) {
			isClzj = false;
			boolean success = false;
			isFromOffline = true;
			if (str == null) {
			...
			if (!sbz) {
				...
				// 巡检模块证件号码字段单独处理
				idcardnumber_s = zjhm;
				onUpdateDisplayXunjianInfo(success, true, true);
			...
			// 初始化mstdc(二维码扫描结果对象)
			msTdc = null;
		} else if (HTTPREQUEST_TYPE_FOR_NORMAL_XUNJIAN_IC == httpRequestType) {
			if (progressDialog != null) {
				progressDialog.dismiss();
				progressDialog = null;
			}
			if (str != null) {
				...
				if (onParseXMLDataByCard(str)) {
					httpReturnXMLInfo = tempHttpReturnXMLInfo;
					setContentView(R.layout.objectdetail);
					Button _btnnoidcard = (Button) findViewById(R.id.btnNoIDCard);
					_btnnoidcard.setOnClickListener(retryInputCardNumListener);
					Button _btnexcept = (Button) findViewById(R.id.btnExceptionRegist_object);
					_btnexcept.setOnClickListener(exceptRegistListener);
					if (SystemSetting.xunJianType != null) {
						flagRegister = false;
						if (SystemSetting.xunJianType.equals("bw")) {
							((TextView) findViewById(R.id.arg1)).setText(Html
									.fromHtml(getString(R.string.bw_name)
											+ "<font color=\"#acacac\">"
											+ xxd_arg1_s + "</font>"));
							((TextView) findViewById(R.id.arg2)).setText(Html
									.fromHtml(getString(R.string.bw_ssmt)
											+ "<font color=\"#acacac\">"
											+ xxd_arg2_s + "</font>"));
							((TextView) findViewById(R.id.arg3)).setText(Html
									.fromHtml(getString(R.string.bw_kbnl)
											+ "<font color=\"#acacac\">"
											+ xxd_arg3_s + "</font>"));
							((TextView) findViewById(R.id.arg4)).setText(Html
									.fromHtml(getString(R.string.bw_zxhz)
											+ "<font color=\"#acacac\">"
											+ xxd_arg4_s + "</font>"));
							((TextView) findViewById(R.id.arg5))
									.setVisibility(View.GONE);
						} else if (SystemSetting.xunJianType.equals("mt")) {
							((TextView) findViewById(R.id.arg1)).setText(Html
									.fromHtml(getString(R.string.mt_name)
											+ "<font color=\"#acacac\">"
											+ xxd_arg1_s + "</font>"));
							((TextView) findViewById(R.id.arg2)).setText(Html
									.fromHtml(getString(R.string.mt_zax)
											+ "<font color=\"#acacac\">"
											+ xxd_arg2_s + "</font>"));
							((TextView) findViewById(R.id.arg3)).setText(Html
									.fromHtml(getString(R.string.mt_gsgs)
											+ "<font color=\"#acacac\">"
											+ xxd_arg3_s + "</font>"));
							((TextView) findViewById(R.id.arg4)).setText(Html
									.fromHtml(getString(R.string.mt_mtgm)
											+ "<font color=\"#acacac\">"
											+ xxd_arg4_s + "</font>"));
							((TextView) findViewById(R.id.arg5))
									.setVisibility(View.GONE);
							...
							((TextView) findViewById(R.id.arg1)).setText(Html
									.fromHtml(getString(R.string.md_name)
											+ "<font color=\"#acacac\">"
											+ xxd_arg1_s + "</font>"));
							((TextView) findViewById(R.id.arg2)).setText(Html
									.fromHtml(getString(R.string.md_wz)
											+ "<font color=\"#acacac\">"
											+ xxd_arg2_s + "</font>"));
							((TextView) findViewById(R.id.arg3)).setText(Html
									.fromHtml(getString(R.string.md_zdmbcbdw)
											+ "<font color=\"#acacac\">"
											+ xxd_arg3_s + "</font>"));
							((TextView) findViewById(R.id.arg4)).setText(Html
									.fromHtml(getString(R.string.md_ss)
											+ "<font color=\"#acacac\">"
											+ xxd_arg4_s + "</font>"));
							((TextView) findViewById(R.id.arg5)).setText(Html
									.fromHtml(getString(R.string.md_mbdsl)
											+ "<font color=\"#acacac\">"
											+ xxd_arg5_s + "</font>"));
							((TextView) findViewById(R.id.arg5))
									.setVisibility(View.VISIBLE);
							...
							((TextView) findViewById(R.id.arg1)).setText(Html
									.fromHtml(getString(R.string.qy_name)
											+ "<font color=\"#acacac\">"
											+ xxd_arg1_s + "</font>"));
							((TextView) findViewById(R.id.arg2)).setText(Html
									.fromHtml(getString(R.string.qy_lx)
											+ "<font color=\"#acacac\">"
											+ xxd_arg2_s + "</font>"));
							((TextView) findViewById(R.id.arg3)).setText(Html
									.fromHtml(getString(R.string.qy_fw)
											+ "<font color=\"#acacac\">"
											+ xxd_arg3_s + "</font>"));
							((TextView) findViewById(R.id.arg4)).setText(Html
									.fromHtml(getString(R.string.qy_xxdz)
											+ "<font color=\"#acacac\">"
											+ xxd_arg4_s + "</font>"));
							((TextView) findViewById(R.id.arg5))
									.setVisibility(View.GONE);
							if (xxd_arg2_s.equalsIgnoreCase("kkqy")
									|| xxd_arg2_s.equalsIgnoreCase("卡口区域")) {
								findViewById(R.id.btnDuty).setVisibility(
										View.VISIBLE);
								((Button) findViewById(R.id.btnDuty))
							...
							((TextView) findViewById(R.id.arg1)).setText(Html
									.fromHtml(getString(R.string.ft_name)
											+ "<font color=\"#acacac\">"
											+ xxd_arg1_s + "</font>"));
							((TextView) findViewById(R.id.arg2)).setText(Html
									.fromHtml(getString(R.string.md_wz)
											...
											+ xxd_arg2_s + "</font>"));
							((TextView) findViewById(R.id.arg3)).setText(Html
									.fromHtml(getString(R.string.ft_zdgkcbdw)
											+ "<font color=\"#acacac\">"
											+ xxd_arg3_s + "</font>"));
							((TextView) findViewById(R.id.arg4)).setText(Html
									.fromHtml(getString(R.string.md_ss)
							...
							((TextView) findViewById(R.id.arg1))
									.setVisibility(View.GONE);
							((TextView) findViewById(R.id.arg2))
									.setVisibility(View.GONE);
							((TextView) findViewById(R.id.arg3))
									.setVisibility(View.GONE);
							((TextView) findViewById(R.id.arg4))
									.setVisibility(View.GONE);
							((TextView) findViewById(R.id.arg5))
									.setVisibility(View.GONE);
							findViewById(R.id.btnExceptionRegist)
									.setVisibility(View.GONE);
						}
					} else {
						((TextView) findViewById(R.id.arg1))
						...
						((TextView) findViewById(R.id.arg5))
								.setVisibility(View.GONE);
						findViewById(R.id.btnExceptionRegist).setVisibility(
								View.GONE);
					}
				} else {
					if (tempHttpReturnXMLInfo != null
							&& tempHttpReturnXMLInfo.length() > 0) {
						// Toast.makeText(ReadcardActivity.this,
						// tempHttpReturnXMLInfo, BaseToast.LENGTH_LONG).show();
						HgqwToast.getToastView(getApplicationContext(),
								tempHttpReturnXMLInfo).show();
					} else {
						// Toast.makeText(ReadcardActivity.this,
	...
	private void toSdsrActivity() {
		...
		intent.putExtra("info", httpReturnXMLInfo);
		if (Binddata == null) {
			intent.putExtra("voyageNumber", "");
		} else {
			intent.putExtra("voyageNumber", (String) Binddata.get("hc"));
		}
		if (sbz) {
			intent.putExtra("objectType", "06");
		} else {
		...
		if (msTdc != null) {
			...
			if (msTdc.getScanType() == ScanDataUtil.SCAN_TYPE_DENG_LUN) {
				...
				if (StringUtils.isNotEmpty(date) && date.length() == 8) {
					StringBuffer dateSb = new StringBuffer(date.substring(0, 4))
							.append("-").append(date.substring(4, 6))
	...
	private void onReadCompleteOnLine() {
		...
		String url = "sendSwipeRecord";
		NetWorkManager.request(this, url, paramsHis,
				HTTPREQUEST_TYPE_FOR_NORMAL_XUNJIAN_ID);
	}
		...
		if (Binddata != null) {
			...
			if (xjddName == null) {
				...
				if ("bw".equals(SystemSetting.xunJianType)) {
					xjddName = SystemSetting.xunJianMTname + "，"
							+ SystemSetting.xunJianName;
				}
			}
		}

		((RadioButton) findViewById(R.id.radio_btn_ic)).setText("巡查地点："
				+ xjddName);
	}
	 ...
	 * */
	private void onUpdateDisplayXunjianInfo(boolean success,
			boolean officecode, boolean isOffline) {
		boolean dkwl = false;
		if (sfdk_s != null && sfdk_s.equals("1")) {
			dkwl = true;
		}
		...
		if (sbz) {
			// 士兵证
			((TextView) findViewById(R.id.Validation_results))
					.setVisibility(View.GONE);
			...
			((TextView) findViewById(R.id.detail_country))
					.setVisibility(View.GONE);
			((TextView) findViewById(R.id.detail_birthday))
					.setVisibility(View.GONE);
			((TextView) findViewById(R.id.detail_cardtype))
					.setVisibility(View.GONE);
			((TextView) findViewById(R.id.detail_cardnum))
					.setVisibility(View.GONE);
			...
			((TextView) findViewById(R.id.detail_sbk_zqdd)).setText(Html
					.fromHtml(getString(R.string.zqdd) + "："
							+ "<font color=\"#acacac\">"
			...
			((TextView) findViewById(R.id.Validation_results)).setText(Html
					.fromHtml(getString(R.string.Valid_results)
							+ "<font color=\"#acacac\">" + httpReturnXMLInfo
							+ "</font>"));
			if (hgzl_s != null && hgzl_s.length() > 0) {
				if (hgzl_s.equals("48")) {
					((TextView) findViewById(R.id.cardtypetitle))
							.setText(getString(R.string.hgzl_denglunz)
									+ getString(R.string.cardinfo));
					((TextView) findViewById(R.id.yxq))
							.setVisibility(View.VISIBLE);
					((TextView) findViewById(R.id.yxq))
							.setText(Html.fromHtml(getString(R.string.yxq)
									+ "<font color=\"#acacac\">"
									+ (yxq_s == null ? "" : yxq_s) + "</font>"));
				} else if (hgzl_s.equals("50")) {
					((TextView) findViewById(R.id.cardtypetitle))
							.setText(getString(R.string.hgzl_dengluz)
									+ getString(R.string.cardinfo));
					// ((TextView)
					// findViewById(R.id.yxq)).setVisibility(View.GONE);
				} else if (hgzl_s.equals("52")) {
					((TextView) findViewById(R.id.cardtypetitle))
							.setText(getString(R.string.hgzl_dakaoz)
									+ getString(R.string.cardinfo));
					((TextView) findViewById(R.id.yxq))
							.setVisibility(View.VISIBLE);
					((TextView) findViewById(R.id.yxq))
					...
					((TextView) findViewById(R.id.cardtypetitle))
							.setText(getString(R.string.hgzl_xdqy)
									+ getString(R.string.cardinfo));
					((TextView) findViewById(R.id.yxq))
							.setVisibility(View.VISIBLE);
					((TextView) findViewById(R.id.yxq))
					...
					((TextView) findViewById(R.id.cardtypetitle))
							.setText(getString(R.string.cardinfo));
					// ((TextView)
					// findViewById(R.id.yxq)).setVisibility(View.GONE);
				}
			...
			if (hgzl_s != null && hgzl_s.length() > 0) {
				if (hgzl_s.equals("48")) {
					...
					((TextView) findViewById(R.id.yxq))
							.setVisibility(View.VISIBLE);
					((TextView) findViewById(R.id.sdcb))
							.setVisibility(View.VISIBLE);
					((TextView) findViewById(R.id.yxq))
							.setText(Html.fromHtml(getString(R.string.yxq)
									+ "<font color=\"#acacac\">"
									+ (yxq_s == null ? "" : yxq_s) + "</font>"));
					((TextView) findViewById(R.id.sdcb)).setText(Html
							.fromHtml(getString(R.string.xunjian_sdcb) + "："
									+ "<font color=\"#acacac\">"
									+ (sdcb_s == null ? "" : sdcb_s)
									+ "</font>"));
				} else if (hgzl_s.equals("50")) {
					...
					((TextView) findViewById(R.id.yxq))
							.setText(Html.fromHtml(getString(R.string.yxq)
									...
									+ (yxq_s == null ? "" : yxq_s) + "</font>"));
					((TextView) findViewById(R.id.sdcb)).setText(Html
							.fromHtml("区域范围" + "：" + "<font color=\"#acacac\">"
									+ (sdcb_s == null ? "" : sdcb_s)
									+ "</font>"));
				} else {
					((TextView) findViewById(R.id.cardtypetitle))
			...
			if (Binddata != null) {
				...
				if ((sex_s != null) && (sex_s.length() > 0)) {
					tempstr = DataDictionary.getDataDictionaryName(sex_s,
							DataDictionary.DATADICTIONARY_TYPE_SEX_TYPE);
				} else {
					tempstr = null;
				}

				((TextView) findViewById(R.id.detail_sex))
						.setText(Html.fromHtml(getString(R.string.sex) + "："
								+ "<font color=\"#acacac\">"
				...
				((TextView) findViewById(R.id.detail_country))
						.setText(Html.fromHtml(getString(R.string.country)
								+ "：" + "<font color=\"#acacac\">"
								+ (tempstr == null ? "" : tempstr) + "</font>"));
				((TextView) findViewById(R.id.detail_birthday)).setText(Html
						.fromHtml(getString(R.string.birthday) + "："
								+ "<font color=\"#acacac\">"
								+ (birthday_s == null ? "" : birthday_s)
								+ "</font>"));
				if ((idcardtype_s != null) && (idcardtype_s.length() > 0)) {
					tempstr = DataDictionary
				...
				((TextView) findViewById(R.id.detail_cardtype))
						.setText(Html.fromHtml(getString(R.string.cardtype)
								+ "：" + "<font color=\"#acacac\">"
								+ (tempstr == null ? "" : tempstr) + "</font>"));
				((TextView) findViewById(R.id.detail_cardnum))
						.setText(Html
				...
				if (officecode) {
					((TextView) findViewById(R.id.detail_office))
							.setText(Html.fromHtml(getString(R.string.office)
									...
									+ (office_s == null ? ""
											: DataDictionary
													.getDataDictionaryOfficeName(
															office_s,
															((hgzl_s != null && hgzl_s
																	.equals("50")) ? DataDictionary.DATADICTIONARY_TYPE_CBYGZW
				...
				// ((TextView)
				// findViewById(R.id.sdcb)).setVisibility(View.GONE);
			} else {
				// 船下
				...
				if (hgzl_s != null
						&& (hgzl_s.equals("48") || hgzl_s.equals("50") || YfZjxxConstant.ZJLX_XDQY
								.equals("hgzl_s"))) {
					((TextView) findViewById(R.id.yxq))
							.setText(Html.fromHtml(getString(R.string.yxqx)
									+ "：" + "<font color=\"#acacac\">"
									+ (yxq_s == null ? "" : yxq_s) + "</font>"));
				} else {
					// ((TextView)
				...
				if (hgzl_s != null && hgzl_s.equals("50")) {
					((TextView) findViewById(R.id.detail_unit))
							.setVisibility(View.GONE);
					((TextView) findViewById(R.id.sdcb)).setText(Html
							.fromHtml(getString(R.string.xunjian_fwcb) + "："
									+ "<font color=\"#acacac\">"
									+ (sdcb_s == null ? "" : sdcb_s)
									+ "</font>"));
				} else {
					((TextView) findViewById(R.id.detail_unit)).setText(Html
							.fromHtml(getString(R.string.unit) + "："
									...
									+ "</font>"));
					// ((TextView)
					// findViewById(R.id.sdcb)).setText(Html.fromHtml(getString(R.string.xunjian_sdcb)
					// + "：" + "<font color=\"#acacac\">"
		...
		ShowViewUtil.showPzbjxx(personInfo, this,
				GlobalFlags.LIST_TYPE_FROM_XUNCHAXUNJIAN, isOffline);
		...
		if (hasImage && netWorkImage != null) {
			...
			// Drawable netWorkImage = ImageFactory.loadDrawable(zjhm);
			// if (netWorkImage == null && cardInfo != null &&
			// cardInfo.getPeople() != null) {
			// netWorkImage =
			// ImageFactory.loadDrawableByPath(cardInfo.getPeople().getPhotoPath());
			// }
			// ImageFactory.setDrawable(imageView_bg, netWorkImage);
			Bitmap netWorkImage = ImageFactory.loadImage(zjhm);
			if (netWorkImage == null && cardInfo != null
					&& cardInfo.getPeople() != null) {
				netWorkImage = ImageFactory.loadImageByPath(cardInfo
						.getPeople().getPhotoPath());
			}
			ImageFactory.setImage(imageView_bg, netWorkImage);
		} else {
			...
			while (type != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.START_TAG:
					if ("result".equals(parser.getName())) {
						...
						// name
						xxd_arg1_s = parser.nextText();
						SystemSetting.xunJianName = xxd_arg1_s;
						...
						if (xxd_arg2_s != null && !"".equals(xxd_arg2_s)) {
							if ("kkqy".equals(xxd_arg2_s)) {
								xxd_arg2_s = getString(R.string.qy_lx_kkqy);
							} else if ("jkqy".equals(xxd_arg2_s)) {
								xxd_arg2_s = getString(R.string.qy_lx_jkqy);
							}
						}
	 ...
	 *            ，true表示代码，需要从数据字典翻译，否则直接显示
	 * */
	private void onUpdateDisplayInfo(boolean success, boolean officecode) {
		// TODO Auto-generated method stub
		boolean dkwl = false;
		...
		if (dkwl) {
			...
			findViewById(R.id.imageView_Traffic_direction).setVisibility(
					View.GONE);
			((TextView) findViewById(R.id.detail_name)).setText(Html
					.fromHtml(getString(R.string.shipname)
							+ "<font color=\"#acacac\">"
			...
			((TextView) findViewById(R.id.detail_birthday)).setText(Html
					.fromHtml(getString(R.string.birthday) + "："
							...
							+ "</font>"));
			if ((idcardtype_s != null) && (idcardtype_s.length() > 0)) {
				tempstr = DataDictionary.getDataDictionaryName(idcardtype_s,
						DataDictionary.DATADICTIONARY_TYPE_CERTIFICATES_TYPE);
			} else {
				tempstr = null;
			}
			...
			((TextView) findViewById(R.id.detail_unit)).setText(Html
					.fromHtml(getString(R.string.unit) + "："
							...
							+ (unit_s == null ? "" : unit_s) + "</font>"));
			// 船员没有"有效期"字段
			if (StringUtils.isNotEmpty(hgzl_s)
					&& (YfZjxxConstant.ZJLX_XDQY.equals(hgzl_s)
							|| YfZjxxConstant.ZJLX_DK.equals(hgzl_s) || YfZjxxConstant.ZJLX_DLUN
								.equals(hgzl_s))) {
				((TextView) findViewById(R.id.detail_yxq))
						.setVisibility(View.VISIBLE);
				((TextView) findViewById(R.id.detail_yxq)).setText(Html
						.fromHtml(getString(R.string.yxqx) + "："
								+ "<font color=\"#acacac\">"
								+ (yxq_s == null ? "" : yxq_s) + "</font>"));
			} else {
				((TextView) findViewById(R.id.detail_yxq))
			...
			if (from.equals("04") && pzxx_s != null && pzxx_s.length() != 0) {
				((TextView) findViewById(R.id.detail_pzxx)).setText(Html
						.fromHtml("<font color=\"#acacac\">" + pzxx_s
								+ "</font>"));
				findViewById(R.id.detail_pzxx).setVisibility(View.VISIBLE);
				findViewById(R.id.detail_pzxx_static).setVisibility(
						View.VISIBLE);
			}
		}
		ImageView imageView_bg = (ImageView) findViewById(R.id.imageView_photo);
				...
				if (from.equals("04")) {
					...
					if (txjlid_s != null && txjlid_s.length() > 0) {
						...
						params.add(new BasicNameValuePair("passDirection",
								(sxcfx_s != null && sxcfx_s.equals("0")) ? "1"
										: "0"));
						...
						progressDialog.setIndeterminate(false);
						progressDialog.show();
						NetWorkManager.request(ReadcardActivity.this, url,
								params,
		...
		if (from.equals("04")) {
			_btnmodifyPassDirection
					.setText(R.string.xunjian_person_save_record);
			...
			if (dkwl || success == false || txjlid_s == null
					|| txjlid_s.length() == 0) {
			...
			/*
			 * if (from.equals("01")) { //
			 * _btngoods.setVisibility(View.INVISIBLE); } else {
			 */
			if ((!dkwl) && success && successFlag == 1 && hgzl_s != null
					&& (txjlid_s != null && txjlid_s.length() > 0)) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case STARTACTIVITY_FOR_REGISTER_PERSONINFO: // 手动保存记录页面返回
			...
			if (resultCode == RESULT_OK) {
				...
				if (from.equals("03") || from.equals("04")) {
					setContentView(R.layout.xunchaxunjiandetail);
					onUpdateDisplayXunjianInfo(true, false, true);
				} else {
					setContentView(R.layout.persondetail);
					onUpdateDisplayInfo(true, false);
					Button _btngoods = (Button) findViewById(R.id.btnRecordGoods);
		...
		case STARTACTIVITY_FOR_ZXING:
			setContentView(R.layout.readcard);
			TextView textView = (TextView) findViewById(R.id.berthcard);
			TextView payTypeTitle = (TextView) findViewById(R.id.payTypeTitle);
			if (textView != null) {
				textView.setText(R.string.idcardnum);
			}
			if (payTypeTitle != null) {
				payTypeTitle.setText(R.string.readCardData);
			}
			/**
			 * 手动查找按钮
			 */
			...
			if (resultCode == RESULT_OK) {
				// 接收扫描二维码返回来的数据
				boolean isback = data.getBooleanExtra(Constant.ZXING_ISBACK,
						false);
				if (!isback) {// 判断是不是从按返回按钮进入该界面
					msTdc = (MsTdc) data
							.getSerializableExtra(Constant.ZXING_DATA);
					if (msTdc != null) {
						zxingInfo = msTdc.getZjhm();
						if (com.pingtech.hgqw.utils.StringUtils
								.isEmpty(zxingInfo)) {
							zxingInfo = "";
						}
					} else {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = mInflater.inflate(
						R.layout.xunchaxunjian_listview_class, null);
	...
	private void readInit() {
		...
		case DeviceUtils.DEVICE_MODEL_MIMA:
			...
			readService = ReadService.getInstent(this, readCardHander,
					ReadService.READ_TYPE_DEFAULT_AND_ICKEY);
			break;
		case DeviceUtils.DEVICE_MODEL_M:
		case DeviceUtils.DEVICE_MODEL_CFON640:
		case DeviceUtils.DEVICE_MODEL_PA8:
	...
	private void readIdSuccess(People people) {
		...
		old_sex_s = sex_s;
		sex_s = DataDictionary.getDataDictionaryCode(people.getPeopleSex(),
				DataDictionary.DATADICTIONARY_TYPE_SEX_TYPE);
	...
	public void offLineResult(Pair<Boolean, Object> res, int offLineRequestType) {
		if (res != null && res.second != null) {
			...
			HgqwToast.getToastView(getApplicationContext(),
					getString(R.string.filed_validate_card)).show();
		}
	}
}
